...
library; this type hierarchy also includes generic mixins for common AST
types across languages.  For instance, `IfStatementAST`, `StatementAST`,
and `IdentifierAST` classes abstract common language constructs
and allow identification of these types of ASTs regardless of the
language of the underlying program.  As with other objects, clients
may use `isinstance` to test if an AST is an instance or subclass of a
given type, as shown below:
