...
(-> int/cut (ast (or ast list)) (values ast &optional))
(defun int/cut (root pt)
  (if-let ((path (if (typep pt 'ast)
                     (ast-path root pt)
                     (python-to-cl-ast-path root pt))))
    (less root path)
    (error "~a is not in ~a" pt root)))

(-> int/insert (ast (or ast list) ast) (values ast &optional))
...
(-> int/replace (ast (or ast list) ast) (values ast &optional))
