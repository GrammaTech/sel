...
lisp-runner: test/lisp-runner.lisp
	buildapp \
		--output lisp-runner \
		--load test/lisp-runner.lisp \
		--entry main

check: $(LISP_FILES) lisp-runner
	sbcl \
		--noinform \
		--eval '(require :software-evolution)' \
...
clean:
	rm -f *.fasl software/*.fasl evolution/*.fasl test/*.fasl \
		test/gcd/*.fasl soft-ev lisp-runner
