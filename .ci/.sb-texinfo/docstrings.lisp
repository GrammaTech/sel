...
(defun collect-comments (file &aux in-comment-p comments)
  "Collect all top-level comment blocks preceded by a page-feed ^L character."
        ...
        (when (> (length line) 0)
          (if in-comment-p
              (if (equal #\; (aref line 0))
                  (push (let ((trimmed (string-trim ";" line)))
