...
a declaration AST, return AST unchanged."
  (flet ((collect-decls (ast)
    ...
    (cond
      ...
      ((and (keywordp type)
            (typep ast 'degenerate-declaration-ast))
       (collect-decls ast))
      ((and (keywordp type) (not (typep ast 'declaration-ast)))
       (collect-decls ast))
      ((and (eql type :variable) (typep ast 'variable-declaration-ast))
       (adjoin ast (collect-decls ast)))
      ((and (eql type :function) (typep ast 'function-declaration-ast))
       (adjoin ast (collect-decls ast)))
      ((and (eql type :type) (typep ast 'type-declaration-ast))
       (adjoin ast (collect-decls ast)))
      ((and (eql type :tag) (typep ast 'type-declaration-ast))
       (adjoin ast (collect-decls ast)))
      ((and (eql type :macro) (typep ast 'macro-declaration-ast))
       (adjoin ast (collect-decls ast)))
      ((and (eql type :namespace) (typep ast 'namespace-declaration-ast))
       (adjoin ast (collect-decls ast)))
      ((and (eql type :method) (typep ast 'function-declaration-ast))
       (adjoin ast (collect-decls ast)))
      ((and (keywordp type) (typep ast 'declaration-ast))
       (list ast))
      ((null type)
...
(defgeneric collect-var-uses (software id)
  ...
  (:method ((obj normal-scope) (id t))
    (collect-var-uses (genome obj) id))
  (:method ((root normal-scope-ast) (id identifier-ast))
    (when-let ((id (get-declaration-id :variable id)))
      (collect-if (lambda (ast)
                    (and (not (eql ast id))
                         (typep ast 'identifier-ast)
                         (eql (get-declaration-id :variable ast) id)
                         ;; Exclude nested identifiers.
                         (not (typep (lookup-parent-ast (attrs-root*) ast)
                                     'identifier-ast))))
                  root)))
  (:method ((root normal-scope-ast) (field field-ast))
    (when-let (id (get-declaration-id :variable field))
      (collect-if (lambda (ast)
                    (and (not (eql ast id))
                         (typep ast '(or identifier-ast field-ast))
                         (eql id (get-declaration-id :variable ast))))
                  root))))
...
`map-union' with `append-longest' as its value union function. You can
override the value union function for your own synthesized attributes
by providing a `:union-fn` option."
  ...
  (:method :context ((ast ast))
    (restart-case
      ...
      (infer-type-as-nil ()
        ...
        nil)))
  (:method (ast)
    (or (let ((expression-type (infer-expression-type ast)))
...
automatically removed.")
  ...
  (:method ((ast terminal-symbol))
    (find-enclosing #'control-flow-node-p (attrs-root*) ast))
  (:method ((ast loop-ast))
...
declaration.")
  (:method ((parent ltr-eval-ast) (sub ast))
    (or (find-next-sibling #'control-flow-node-p parent sub)
        parent))
  (:method ((parent ast) (child ast))
    (or (if (ltr-eval-ast-p parent)
            (find-next-sibling #'control-flow-node-p parent child)
            (remove child
                    (filter #'control-flow-node-p
                            (children parent))))
        parent))
...
node in the block.")
  ...
  (:method :around ((ast compound-ast))
    (find-if #'control-flow-node-p (children ast)))
  (:method ((ast ast))
    (if (ltr-eval-ast-p ast)
        (first-subexpression (children ast))
        (filter #'control-flow-node-p (children ast))))
  ...
  (:method :context ((type symbol) root (id identifier-ast))
    ...
    (let ((ns (decl-type-namespace type))
      ...
      (or
       ...
       (call-next-method))))
  (:method ((type (eql 'function-declaration-ast)) root id)
    (find-enclosing #'function-declaration-p (attrs-root*) id))
  (:method ((type (eql 'variable-declaration-ast)) root id)
    (find-enclosing #'variable-declaration-p (attrs-root*) id))
