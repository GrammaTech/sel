        ...
        (if (no entry-points) defaults
            (let ((final-exits (final-exits entry-points)))
              ...
              (or (flatten (substitute defaults root final-exits))
                  defaults))))))
  (:method ((ast ast) &aux (root (attrs-root*)))
    (let ((parent (lookup-parent-ast root ast)))
      (if (ltr-eval-ast-p parent)
          (or (find-next-sibling t root ast)
              parent)
          (or (remove ast (children parent))
              parent))))
...
node in the block.")
  ...
  (:method :around ((ast compound-ast))
    (find-if (of-type 'subexpression-ast)
             (children ast)))
  (:method ((ast ast))
