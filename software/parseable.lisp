...
(defmethod slot-unbound
    ((class t) (node conflict-ast) (slot (eql 'ft:descendant-map)))
  (let ((sn (ft:serial-number node)))
    (setf (slot-value node 'ft:descendant-map)
          (vector (list (cons sn sn) nil)))))
...
(defgeneric ancestor-of-p (obj descendant ancestor)
  (:documentation "Returns T if ANCESTOR is an ancestor of DESCENDANT in OBJ.")
  (:method ((obj parseable) descendant ancestor)
    (unless (eq descendant ancestor)
      (shares-path-of-p obj descendant ancestor)))
  (:method ((root ast) descendant ancestor)
    (unless (eq descendant ancestor)
      (shares-path-of-p root descendant ancestor))))

(defun descendant-of-p (obj ancestor descendant)
