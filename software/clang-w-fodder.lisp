...
(defmethod mutate ((clang-w-fodder clang-w-fodder))
  ...
  (let ((op (case (random-elt '(cut insert swap set-value insert-value))
              ...
              (set-value    `(:set-value ,(pick-good ast) ,(pick-json clang-w-fodder)))
              (insert-value `(:insert-value ,(pick-good ast) ,(pick-json clang-w-fodder))))))
    (apply-mutation clang-w-bytes op)
    (values ast op)))
