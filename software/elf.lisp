...
(defmethod mutate ((elf elf-sw) &key pick-good pick-bad)
  "Randomly mutate ELF."
  (setf (fitness elf) nil)
  (flet ((pick-rand () (random (length (genome elf)))))
    (let* ((pick-good (or pick-good #'pick-rand))
           (pick-bad  (or pick-bad  #'pick-rand))
           (mut (case (random-elt '(cut insert swap #|d-cut d-insert d-swap|#))
