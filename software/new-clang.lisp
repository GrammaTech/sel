...
(defgeneric offset (obj)
  ...
  (:method ((obj new-clang-macro-loc))
    (if (< (offset (new-clang-macro-loc-expansion-loc obj))
...
(defmethod ast-includes-in-current-ast ((obj new-clang) (ast new-clang-ast))
  (nest (remove-if #'null)
        (append (nest (mapcar (lambda (ast)
                ...
                (when (and (ast-type ast)
                           (type-i-file (ast-type ast)))
                  (list (type-i-file (ast-type ast))))
                (when (and (ast-attr ast :macro)
                           (new-clang-macro-i-file (ast-attr ast :macro)))
...
(defgeneric ast-file* (ast macro?)
  ...
  (:method ((loc new-clang-macro-loc) macro?)
    (ast-file*
     (if (if (< (offset (new-clang-macro-loc-expansion-loc loc))
