...
(defmethod initialize-instance :after
    ((self c/cpp-true) &key &allow-other-keys)
  (unless (text self)
    (setf (text self) "true")))

(defmethod initialize-instance :after
    ((self c/cpp-false) &key &allow-other-keys)
  (unless (text self)
    (setf (text self) "false")))
...
(defmethod aliasing-table ((id c/cpp-ast))
  (flet ((record-alias (alias aliasee)
           (ch-map (:aliasee->alias
                    (ch-map (alias aliasee)))
                   (:alias->alias-set
                    (ch-map (aliasee (ch-set alias)))))))
    (if (typep id 'identifier-ast)
        (ematch (get-initialization-ast id)
          ((c/cpp-init-declarator (lhs lhs) (rhs rhs))
           (let ((result (initializer-aliasee (attrs-root*) lhs rhs)))
             (if (eql result :failure)
                 (if-let (decl (get-declaration-id :variable id))
                   (record-alias id decl)
                   (empty-ch-map))
                 (record-alias id result))))
          ((c/cpp-assignment-expression
            (lhs lhs)
            (rhs (and rhs (identifier-ast))))
           (if-let (aliasee (aliasee rhs))
             (if-let (decl-id (get-declaration-id :variable lhs))
               (record-alias decl-id aliasee)
               (empty-ch-map))
             (empty-ch-map)))
          (otherwise
           (empty-ch-map)))
                 ...
                 (or (some (lambda (type)
                             (let ((ntype (ntype type)))
                               (and (typep n (equivalent-lisp-type ntype))
                                    ntype)))
                           possible-type-names)
                     ;; If the number doesn't fit, assign it to the
                     ;; largest possible type.
                     (ntype (lastcar possible-type-names)))))
