...
The OBJECT field is a list in which PRED is applied to every element."))

(defclass some-test-task (task)
  ((pred :initarg :pred :accessor pred
         :documentation "Predicate used by `some'."))
  (:documentation "Task to apply a predicate to an item in a list.
Created by `some-task'. The OBJECT is a list item to which PRED is applied."))
...
predicate PRED in TASK succeeds or there are no more items in the list."
...
(defmethod process-task ((task some-test-task) runner)
  "Process a single `some-test-task' by applying PRED to the OBJECT in TASK.
...
`pred'."
  (when (not (task-runner-results runner))
    (when-let ((result (funcall (pred task) (task-object task))))
