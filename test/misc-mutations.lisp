...
(deftest (pick-for-loop-works :long-running) ()
  (with-fixture scopes-clang
    ...
    (let ((var (copy *scopes*)))
      ...
      (is (not (string= (genome-string var)
                        (genome-string *scopes*)))
          "Exploded for loop changes genome.")
      (is (not (scan (quote-meta-chars "for") (genome-string var)))
...
(deftest (explode-for-loop-mutation-works :long-running) ()
  ...
  (let ((simple-loop       (from-file (make-instance 'clang)
    ...
    (flet ((run-factorial (obj n)
      ...
      (is (not (scan (quote-meta-chars "for")
                     (genome-string simple-loop))))
      (is (not (scan (quote-meta-chars "for")
                     (genome-string no-initialization))))
      (is (not (scan (quote-meta-chars "for")
                     (genome-string no-conditional))))
      (is (not (scan (quote-meta-chars "for")
                     (genome-string no-increment))))
      (is (not (scan (quote-meta-chars "for")
                     (genome-string no-body)))))))
...
(deftest (pick-while-loop-works :long-running) ()
  (with-fixture scopes-clang
    ...
    (let ((var (copy *scopes*)))
      ...
      (is (not (string= (genome-string var) (genome-string *scopes*)))
          "Coalesced while loop changes genome.")
      (is (not (scan (quote-meta-chars "while") (genome-string var)))
