...
(deftest infer-type-in-initializer ()
  (let* ((string (fmt "typedef struct foo_s { int x; } foo_t;~%~
void f(foo_t* p) { char c = p->x; }~%"))
         (c-code (from-string 'c string)))
    (is (equal (mapcar [#'source-text #'cadr] (types-in-thing c-code c-code))
               '("void(foo_t*)" "foo_t*" "char" "int" "foo_t*")))))
...
(deftest test-int-expression-type ()
  (macrolet ((test-type (type string)
    ...
    (test-type "unsigned long int" "1lu")
    (test-type "unsigned int" "1u")
    (test-type "unsigned long int" "1lu")
    (test-type "unsigned long long int" "1llu")
