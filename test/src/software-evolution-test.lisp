...
(deftest type-of-var-returns-correct-type ()
  (with-fixture type-of-var-clang
    (let ((var-type1 (type-of-var *soft* "a"
      ...
      (is (equal "[][]" (type-array var-type1)))
      (is (equal ""     (type-array var-type2)))
      (is (equal "[][]" (type-array var-type3)))
      (is (equal "[][]" (type-array var-type4)))
...
(deftest pick-for-loop-works ()
  (with-fixture scopes-clang
    (is (eq :ForStmt (->> (se::pick-for-loop *scopes*)
                          (aget :stmt1)
                          (ast-class)))
...
(deftest pick-while-loop-works ()
  (with-fixture scopes-clang
    (is (eq :WhileStmt (->> (se::pick-while-loop *scopes*)
                            (aget :stmt1)
                            (ast-class)))
